...
void BRIEF::computeCombineAngle(const cv::Mat &image, const std::vector<cv::KeyPoint> &points,
                                vector<bitset> &descriptors, bool treat_image) const {
  const float sigma = 2.f;
  const cv::Size ksize(9, 9);
  ...
  for (kit = points.begin(); kit != points.end(); ++kit, ++dit) {
    // dit->resize(m_bit_length);
    dit->reset();
    float angle = (float)(*kit).angle * factorPI;
    ...
    for (unsigned int i = 0; i < m_x1.size(); ++i) {
      ...
      // * (center + iy * step + ix) );
      x1 = (int)(kit->pt.x +
                 cvRound(m_x1[i] * a - m_y1[i] * b));  //(kit->pt.x + m_x1[i]);//(kit->pt.x +
                                                       //cvRound(m_x1[i]*a-m_y1[i]*b));
      y1 = (int)(kit->pt.y +
                 cvRound(m_x1[i] * b + m_y1[i] * a));  //(kit->pt.y + m_y1[i]);//(kit->pt.x +
                                                       //cvRound(m_y1[i]*b+m_y1[i]*a));
      x2 = (int)(kit->pt.x +
                 cvRound(m_x2[i] * a -
                         m_y2[i] * b));  //(kit->pt.x + m_x2[i]);//(kit->pt.x +
                                         //cvRound(m_x2[i]*a-m_y2[i]*b)); //m_x2[i]*a+m_y2[i]*b)
      y2 = (int)(kit->pt.y +
                 cvRound(m_x2[i] * b +
                         m_y2[i] * a));  //(kit->pt.y + m_y2[i]);//(kit->pt.x +
                                         //cvRound(m_x2[i]*a+m_y2[i]*b)); //m_y2[i]*b-m_y2[i]*a)
