...
class TEBLID_Impl CV_FINAL : public TEBLID {
 public:
  ...
  // returns the descriptor length in bytes
  int descriptorSize() const CV_OVERRIDE {
    return impl.descriptorSize();
  }

  // returns the descriptor type
  int descriptorType() const CV_OVERRIDE {
    return impl.descriptorType();
  }

  // returns the default norm type
  int defaultNorm() const CV_OVERRIDE {
    return impl.defaultNorm();
  }
...
#include "wl_params.h"
    ...
    CV_Error(Error::StsBadArg,
             "n_bits should be either TEBLID::SIZE_512_BITS or TEBLID::SIZE_256_BITS");
  }
}
 ...
 */
static inline bool isKeypointInTheBorder(const cv::KeyPoint& kp, const cv::Size& imgSize,
                                         const cv::Size& patchSize = {32, 32},
                                         float scaleFactor = 1) {
  // This would be the correct measure but since we will compare with half of the size, use this as
  // border size
...
static inline void rectifyABWL(const std::vector<WeakLearnerT>& wlPatchParams,
                               std::vector<WeakLearnerT>& wlImageParams, const cv::KeyPoint& kp,
                               float scaleFactor = 1,
 ...
 * @param integralImage The integral image used to compute the average gray value in the square
 * regions.
...
template <class WeakLearnerT>
void BEBLID_Impl<WeakLearnerT>::compute(InputArray _image, vector<KeyPoint>& keypoints,
...
// constructor
template <class WeakLearnerT>
BEBLID_Impl<WeakLearnerT>::BEBLID_Impl(float scale_factor,
                                       const std::vector<WeakLearnerT>& wl_params)
...
template <class WeakLearnerT>
void BEBLID_Impl<WeakLearnerT>::computeBoxDiffsDescriptor(
    const cv::Mat& integralImg, const std::vector<cv::KeyPoint>& keypoints, cv::Mat& descriptors) {
...
#include "wl_params.h"
    ...
    CV_Error(Error::StsBadArg,
             "n_bits should be either BEBLID::SIZE_512_BITS or BEBLID::SIZE_256_BITS");
  }
}
