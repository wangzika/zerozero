...
class TemplatedDatabase {
 public:
   ...
   */
  template <class T>
  inline void setVocabulary(std::shared_ptr<const T> voc);
   ...
   */
  template <class T>
  void setVocabulary(std::shared_ptr<const T> voc, bool use_di, int di_levels = 0);

  /**
   * Returns a pointer to the vocabulary used
   * @return vocabulary
   */
  inline std::shared_ptr<const TemplatedVocabulary<TDescriptor, F>> getVocabulary() const;
   ...
   */
  EntryId add(const std::vector<TDescriptor> &features, BowVector *bowvec = NULL,
   ...
   *   < 0 means all
   */
  void query(const std::vector<TDescriptor> &features, QueryResults &ret, int max_results = 1,
             int max_id = -1) const;
 ...
 protected:
  ...
  std::shared_ptr<TemplatedVocabulary<TDescriptor, F>> m_voc;
...
template <class TDescriptor, class F>
TemplatedDatabase<TDescriptor, F>::TemplatedDatabase(const std::string &filename) : m_voc(nullptr) {
  load(filename);
}
...
template <class TDescriptor, class F>
TemplatedDatabase<TDescriptor, F>::TemplatedDatabase(const char *filename) : m_voc(nullptr) {
  load(filename);
}
...
template <class TDescriptor, class F>
template <class T>
inline void TemplatedDatabase<TDescriptor, F>::setVocabulary(std::shared_ptr<const T> voc) {
  //m_voc = voc;
  m_voc = std::const_pointer_cast<T>(voc);
  clear();
}
...
template <class TDescriptor, class F>
template <class T>
inline void TemplatedDatabase<TDescriptor, F>::setVocabulary(std::shared_ptr<const T> voc, bool use_di, int di_levels) {
  // std::cout<<"voc ptr =  "<< voc.get() << std::endl;
  m_use_di = use_di;
  m_dilevels = di_levels;
  m_voc = std::const_pointer_cast<T>(voc);
  // std::cout << "m_voc ptr = " << m_voc.get() << std::endl;
  clear();
}
...
template <class TDescriptor, class F>
inline std::shared_ptr<const TemplatedVocabulary<TDescriptor, F>>
...
void TemplatedDatabase<TDescriptor, F>::query(const std::vector<TDescriptor> &features,
                                              QueryResults &ret, int max_results,
                                              int max_id) const {
  BowVector vec;
...
void TemplatedDatabase<TDescriptor, F>::load(const cv::FileStorage &fs, const std::string &name) {
  ...
  if (!m_voc)
     m_voc = std::make_shared<TemplatedVocabulary<TDescriptor, F>>();
