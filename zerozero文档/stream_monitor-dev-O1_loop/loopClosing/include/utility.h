...
class Utility {
 public:
  ...
  static Eigen::Matrix<typename Derived::Scalar, 3, 1> logSO3(const Eigen::MatrixBase<Derived> &R) {
    ...
    const Scalar_t tr = R(0, 0) + R(1, 1) + R(2, 2);
    Eigen::Matrix<Scalar_t, 3, 1> w;
    w << (R(2, 1) - R(1, 2)) / 2, (R(0, 2) - R(2, 0)) / 2, (R(1, 0) - R(0, 1)) / 2;
    const Scalar_t costheta = (tr - 1.0) * 0.5f;
    if (costheta > 1 || costheta < -1)
      return w;
    const Scalar_t theta = acos(costheta);
    const Scalar_t s = sin(theta);
    if (fabs(s) < 1e-5)
      Scalar_t w;
    else
      return theta * w / s;
