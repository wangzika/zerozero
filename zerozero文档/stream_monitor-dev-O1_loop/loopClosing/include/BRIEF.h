...
class BRIEF {
 ...
 public:
  ...
  inline void operator()(const cv::Mat &image, const std::vector<cv::KeyPoint> &points,
                         std::vector<bitset> &descriptors, bool usingAngle,
                         bool treat_image) const {
    if (usingAngle)
      computeCombineAngle(image, points, descriptors, treat_image);
    else
      compute(image, points, descriptors, treat_image);
  }
  /**
  ...
  void compute(const cv::Mat &image, const std::vector<cv::KeyPoint> &points,
               std::vector<bitset> &descriptors, bool treat_image = true) const;
  void computeCombineAngle(const cv::Mat &image, const std::vector<cv::KeyPoint> &points,
                           std::vector<bitset> &descriptors, bool treat_image = true) const;
  /**
